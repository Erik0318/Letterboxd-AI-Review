...
function buildPrompt(body: Body): { system: string; user: string } {
  ...
  const system =
    ...
    `Use concrete references to merged master-table metrics (rating contradictions, era preference, rewatches, unrated behavior, review language). ` +
    `If anomaly.importSpikeDetected=true, NEVER treat import-day spikes as real watch binges; only use watched_at/logged_at distributions for pace judgments. ` +
